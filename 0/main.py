...
# import modules.manage as server
# from modules.whereami import whereAmI
...
def browserSetup(isMobile: bool, user_agent: str = PC_USER_AGENT, proxy: str = None) -> WebDriver:
    ...
    if 0 == 1:
        ...
        if isProxyWorking(proxy):
            ...
            #dbwork.sendMessageToTelegram(f"proxy is not working : {proxy}" , 'all mail')
            input("please enter valid proxy")
...
def login(browser: WebDriver, email: str, pwd: str, isMobile: bool = False):
    ...
    if ARGS.session:
        ...
        if browser.title == 'Microsoft account | Home' or isElementExists(browser, By.ID, 'navs_container'):
            ...
            serverAccount.lock(CURRENT_ACCOUNT)
            #dbwork.updateLockedAccount(email)
            FINISHED_ACCOUNTS.append(CURRENT_ACCOUNT)
            updateLogs()
            cleanLogs()
            # locked account detect from here
            ...
            if isElementExists("iShowSkip") and ARGS.skip_unusual:
                ...
                if ARGS.telegram or ARGS.discord:
                    message = createMessage(CURRENT_ACCOUNT)
                    print(message, "214")
...
def RewardsLogin(browser: WebDriver):
    ...
    try:
        ...
        if browser.find_element(By.XPATH, '//*[@id="error"]/h1').get_attribute(
            ...
            updateLogs()
            FINISHED_ACCOUNTS.append(CURRENT_ACCOUNT)
            #dbwork.updateSuspend(CURRENT_ACCOUNT)
            raise Exception(
...
def logs():
    ...
    try:
        ...
        for user in ACCOUNTS:
            shared_items.append(user['email'])
            if not user['email'] in LOGS.keys():
                LOGS[user["email"]] = {"Last check": "",
                                          "Today's points": 0,
                                          "Points": 0}
...
def finishedAccount():
    ...
    print("account finished = ", CURRENT_ACCOUNT)
    New_points = POINTS_COUNTER - STARTING_POINTS
    serverAccount.updatePoint(CURRENT_ACCOUNT, POINTS_COUNTER , New_points)
    ##dbwork.finishAccount(CURRENT_ACCOUNT, POINTS_COUNTER,GRIND , New_points)
...
def createMessage():
    ...
    for index, value in enumerate(LOGS.items(), 1):
        ...
        if value[1]['Last check'] == str(date.today()):
            ...
            serverAccount.block(CURRENT_ACCOUNT)
            status = '❌ Suspended'
            message += f"{index}. {value[0]}\n📝 Status: {status}\n\n"
...
def sendToTelegram(message):
    ...
    if len(message) > 4096:
        ...
        for ms in messages:
            t.notify(
                message=ms, token="6286895226:AAHURnkz9EQJKghm53n4F5eI-Hl068Y6ZvU", chat_id="1711144420")
    else:
        t.notify(message=message,
                 token="6286895226:AAHURnkz9EQJKghm53n4F5eI-Hl068Y6ZvU", chat_id="1711144420")
...
""")
    prRed(
        f"            by @whitestorm upgraded by @whitestorm        version {version}\n")
...
def farmer():
    ...
    try:
        ...
        for account in ACCOUNTS:
            ...
            if account["lockdown"]:
                cleanLogs()
                LOGS[CURRENT_ACCOUNT]['Last check'] = 'Your Account is in lockdown mode'
                LOGS[CURRENT_ACCOUNT]["Today's points"] = 'N/A'
                LOGS[CURRENT_ACCOUNT]["Points"] = 'N/A'
                updateLogs()
                serverAccount.lockdown(CURRENT_ACCOUNT)
                ##dbwork.sendMessageToTelegram("bypass account due to lockdown mode", CURRENT_ACCOUNT)
            if CURRENT_ACCOUNT in FINISHED_ACCOUNTS or account["lockdown"] == True:
                continue
            ...
            if not LOGS[CURRENT_ACCOUNT]['PC searches']:
                ...
                login(browser, account['email'], account['password'])
            ...
            if MOBILE:
                ...
                login(browser, account['email'], account['password'], True)
                prGreen('[LOGIN] Logged-in successfully !')
                if LOGS[account['email']]['PC searches'] and ERROR:
                    STARTING_POINTS = POINTS_COUNTER
                    browser.get(BASE_URL)
                    waitUntilVisible(browser, By.ID, 'app-host', 30)
                    redeem_goal_title, redeem_goal_price = getRedeemGoal(
        ...
        if "executable needs to be in PATH" in str(e):
            ...
            prRed(str(e))
            ##dbwork.sendMessageToTelegram("some error occur in program", "---")
...
def main():
    ...
    #server.main()
    #GRIND.append(whereAmI())
    ###dbwork.sendMessageToTelegram("grind started :- ", "all emails")
    POINTS = 0
...
if __name__ == '__main__':
    try:
        ...
        server.error(CURRENT_ACCOUNT,str(e),root_folder)
        input("press Enter to close...")
